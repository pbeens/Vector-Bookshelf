...
function App() {
  ...
  const [initialUtility, setInitialUtility] = useState(null)
  ...
  // Fix: Restore focus to search input when Utilities modal closes to prevent "freeze" state
  useEffect(() => {
    if (!showUtilities && !showTaxonomyDoctor && !showRulesEditor) {
      // Small timeout to allow React to re-enable/mount elements
      setTimeout(() => {
        const searchInput = document.getElementById('main-search-input');
        if (searchInput) {
          searchInput.disabled = false;
          // searchInput.focus(); // Optional: might be annoying if auto-focused
        }
      }, 100);
    }
  }, [showUtilities, showTaxonomyDoctor, showRulesEditor]);
  ...
  return (
    <div className="min-h-screen bg-background text-[var(--color-text-main)] font-sans selection:bg-indigo-500/30 relative transition-colors duration-300">
      <div className="w-[95%] max-w-[1600px] mx-auto px-6 py-12">
        {/* Toggle Theme Button */}
        <div className="absolute top-6 left-6 z-50">
          <div className="flex gap-2">
            ...
            <button
              ...
              style={{
                backgroundColor: showUtilities ? 'rgb(6, 182, 212)' : (darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'),
                color: showUtilities ? 'black' : (darkMode ? 'white' : 'rgb(55, 65, 81)'),
                borderColor: showUtilities ? 'rgb(34, 211, 238)' : (darkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)')
              }}
              className="p-2 rounded-full backdrop-blur-md border shadow-lg transition-all duration-300 group"
        ...
        <div className="absolute top-6 right-6 flex flex-col gap-2 z-50">
          ...
          <button
            onClick={() => {
              setInitialUtility('ai-manager');
              setShowUtilities(true);
            }}
            title="Configure AI Models"
            className="flex items-center gap-2 bg-white/80 dark:bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-black/5 dark:border-white/10 shadow-lg transition-all duration-300 hover:bg-white dark:hover:bg-black/60 cursor-pointer"
          >
        ...
        <main className="space-y-8">
          {showUtilities ? (
            <Utilities
              initialUtilityId={initialUtility}
              onClose={() => { setShowUtilities(false); setInitialUtility(null); }}
              scanProps={{
                path,
                setPath,
                startScan,
                      ...
                      <tbody className="divide-y divide-black/5 dark:divide-white/5">
                        {paginatedBooks.map((book) => (
                          ...
                          >
                                        ...
                                        return (
                                          <button
                                            ...
                                            className={`px-2 py-0.5 rounded-md text-xs font-medium transition-all cursor-pointer border ${isActive
                                              ...
                                              : 'bg-black/5 dark:bg-white/5 text-secondary border-black/5 dark:border-white/10 hover:bg-emerald-500/10 hover:text-emerald-600 dark:hover:text-emerald-300 hover:border-emerald-500/30'
                                              }`}
                                          >
                                            {author}
                                          </button>
                            ...
                            <td className="px-4 py-3 font-mono text-[11px] break-words relative group">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={async () => {
                                    try {
